...
resource "proxmox_virtual_environment_file" "caddy_hook" {
  ...
  source_raw {
    data      = file("${path.module}/files/caddy-hook.sh")
    file_name = "${var.container_hostname}-caddy-hook.sh"
  }
}

resource "proxmox_virtual_environment_file" "caddy_compose" {
  content_type = "snippets"
  datastore_id = var.snippets_datastore_id
  node_name    = var.node_name
  file_mode    = "0644"
  ...
  source_raw {
    ...
    file_name = "caddy-docker-compose.yml"
  }
}

resource "proxmox_virtual_environment_file" "caddyfile" {
  content_type = "snippets"
  datastore_id = var.snippets_datastore_id
  node_name    = var.node_name
  file_mode    = "0644"
  ...
  source_raw {
    data      = file("${path.module}/files/Caddyfile")
    file_name = "caddy-Caddyfile"
  }
}
...
resource "proxmox_virtual_environment_container" "caddy" {
  ...
  hook_script_file_id = proxmox_virtual_environment_file.caddy_hook.id
  protection          = false
  ...
  depends_on = [
    proxmox_virtual_environment_file.caddy_compose,
    proxmox_virtual_environment_file.caddyfile
  ]
}
